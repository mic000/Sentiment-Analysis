...
def verify_against_sklearn(X):
    """
    Validation PCA comparison
    """
    from sklearn.decomposition import PCA as SklearnPCA

    print("\n" + "=" * 55)
    print("  validation PCA: mine vs sklearn")
    print("=" * 55)

    # myPCA
    my_pca = MyPCA(n_components=5)
    X_mine = my_pca.fit_transform(X)
    ...
    print(f"    myPCA:   {my_pca.eigenvalues_[:5]}")
    print(f"    sklearn: {sk_pca.explained_variance_[:5]}")
    print(f"\n  explained variance comparison:")
    print(f"    myPCA:   {my_pca.explained_variance_ratio_[:5]}")
    print(f"    sklearn: {sk_pca.explained_variance_ratio_[:5]}")

    # result value comparison
    diff = np.abs(np.abs(X_mine) - np.abs(X_sklearn)).max()
    print(f"\n  Difference in projection results: {diff:.2e}")
    print(f"  {'✓ Verification passed' if diff < 1e-8 else '⚠ error on comparison'}")
...
if __name__ == "__main__":
    ...
    tfidf = tfidf_features(dataset[0], dataset[1])

    # Demo 1: 基本使用
    print("=" * 55)
    print("  Demo 1: 手搓 PCA 基本使用")
    print("=" * 55)
    pca = MyPCA()
    X_reduced = pca.fit_transform(dataset[0])
    print(f"\n原始形状: {dataset[0].shape}")
    print(f"降维后形状: {X_reduced.shape}")
    print(f"方差解释率: {sum(pca.explained_variance_ratio_) * 100:.2f}%")
    ...
    reconstruction_error = np.mean((dataset[0] - X_reconstructed) ** 2)
    print(f"  原始形状: {dataset[0].shape}")
    print(f"  重构形状: {X_reconstructed.shape}")
    print(f"  平均重构误差 (MSE): {reconstruction_error:.4f}")
    print(f"  (误差不为 0 是正常的，因为降维丢了 {20 - 5}/{20} 的维度)")

    # Demo 3: 对比 sklearn
    verify_against_sklearn(dataset[0])